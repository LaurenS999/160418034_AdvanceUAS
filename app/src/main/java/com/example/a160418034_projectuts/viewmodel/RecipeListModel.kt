...
class RecipeListModel(application: Application): AndroidViewModel(application), CoroutineScope  {
    ...
    fun FecthByKategori(kategori:String){
        loadingLD.value = true
        RecipeLoadErrorLD.value = false
        launch {
            val db = buildDb(getApplication())
            RecipeLD.value = db.recipeDao().selectRecipeByKategori(kategori)
            loadingLD.value = false
        }
    }
    ...
    fun favorite(){
        ...
        launch {
            val db = buildDb(getApplication())
            RecipeLD.value = db.recipeDao().selectRecipeByFavorite()
            loadingLD.value = false
        }
    }
